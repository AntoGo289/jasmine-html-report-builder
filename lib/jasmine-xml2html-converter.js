...
var HTMLReport = function() {
    ...
    var createBrowserPage = function(browserConfigLocation) {
        ...
        htmlReportBody += '<table id="td-table">' + thisConfig.testDetails + '</table></body></html>';
    ...
    var createOverviewPage = function() {
        ...
        var htmlFooter = '<footer><div class="footerMsg">Report created using <i><b>Jasmine-HTML-Report-Builder</b></i> node module</div></footer></html>'
            // If specs are to be included in overview page, add them in, otherwise finish body of page
        htmlReportBody += config.includeSpecsInOverview ? '<table id="td-table">' + config.testDetails + '</table></body>' + htmlFooter : '</body>' + htmlFooter;
        // Combine html coponents
        var htmlReport = htmlReportHead + htmlReportSummary + htmlReportBody;
        // Prepare to write new .html file
